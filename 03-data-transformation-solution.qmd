...
Seskupte data v tabulce `babynames` podle roku a potom ocislujte data podle poctu. Vyfiltruje pouze vysledky, kde `rank == 1` (nejvyssi hodnota dany rok).
...
```{r}
ggplot(df, aes(year, name, color = name)) +
  geom_point()
```
...
```{r}
babynames |> 
  summarise(n_mean = mean(n), n_var = var(n))
```

```{r}
babynames |> 
  mutate(n_mean = mean(n), n_sd = sd(n)) |> 
  filter(n >= n_mean - 2*n_sd,n  <= n_mean + 2*n_sd)
```
